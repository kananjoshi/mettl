...
class MettlXBlock(XBlock):
    ...
    display_name = String(
        ...
        default = "Mettl XBlock",
    )

    attempts = Integer(
        display_name = "Attempts",
        help = "Defines the number of times a student can attempt the test.",
        scope = Scope.settings,
        default = 0
    )
    ...
    weight = Float(
        display_name = "Weight",
        help = "Defines the number of points this test is worth",
        scope = Scope.settings, default = 100,
        values={"min":0, "step":.1}
    )

    current_assessment_id = Integer(
        display_name = "Current Assessment ID",
        help = "Select one of the above assessment id.",
        scope = Scope.settings,
        default = ""
    )

    test_button_label = String(
        display_name = "Test Button Label",
        help = "Defines the label for the test button",
        scope = Scope.settings,
        default = "Click here to start the test"
    )

    test_button_back_color = String(
        display_name = "Test Button Background Color",
        help = "Defines the background color of the test button",
        scope = Scope.settings,
        default = "#00ACCD"
    )

    test_button_color = String(
        display_name = "Test Button Text Color",
        help = "Defines the text color of the test button",
        scope = Scope.settings,
        default = "#FFFFFF"
    )
    ...
    def create_schedules_for_assessment(self):
        ...
        for attempt_no in range(int(self.attempts)):
            ...
            ts = str(time.time())
            sc = {
                    "assessmentId": self.current_assessment_id,
                    "name": "Test " + str(attempt_no+1), 
                    "scheduleType": "AlwaysOn",
                    "webProctoring": {"enabled": False}, 
                    "access": { "type": "OpenForAll", "sendEmail": False}, 
                    "ipAccessRestriction":{"enabled": False}, 
                    "sourceApp":"Mettl" 
                }
            ...
            if self.current_assessment_id:        
                ...
                if not self.has_any_error():
                    ...
                    for schedule, schedule_name in sorted(schedules.iteritems(), key=lambda (k,v): (v,k)):
                        all_students_data[schedule] = [schedule_name]
                        students = self.get_all_student_details_in_schedule(schedule)
                        if not self.has_any_error():
                            for student in students:
                                if student['testStatus']['status'] == "Completed":
                                    all_students_data[schedule].append({
                                        'first_name': student['registration']['First Name'],
    ...
    def studio_view(self, context=None):
        ...
        edit_fields = (
                (field, none_to_empty(getattr(self, field.name)), input_type)
                for field, input_type in (
